...
public class Skeleton<T>
{
    ...
    public Skeleton(Class<T> c, T server)
    {
    	...
    	if (!representsRemoteInterface(c, server))
	 ...
	 * @return
	 */
	private boolean representsRemoteInterface(Class<T> c, T server)
	{
		if (!c.isInterface())
			return false;
	    		...
	    		public void run() {
	    			while (true) {
	    				try
						{
	    					...
	    					serverThread = new ServerThread<T>(socket, c, server, Skeleton.this);
							serverThread.start();
						} catch (IOException e)
						{
			    			listen_error(e);
